...
impl CorpusProcessingResult {
    pub fn print_summary(&self) {
        println!("📊 Corpus Processing Summary:");
        println!("   Documents processed: {}", self.documents_processed);
        println!("   Entities linked: {}", self.entities_linked);
        println!("   Relations identified: {}", self.relations_identified);
        println!("   Clustering efficiency: {:.1}%",
            self.get_clustering_efficiency() * 100.0);
    }
