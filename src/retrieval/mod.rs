...
pub enum QueryType {
    EntityFocused, // Queries about specific entities
    Conceptual,    // Abstract concept queries
    Factual,       // Specific fact retrieval
...
pub enum QueryIntent {
    Overview,    // High-level summary needed
    Detailed,    // Detailed information needed
...
impl RetrievalSystem {
    ...
    pub fn new(config: &Config) -> Result<Self> {
        let retrieval_config = RetrievalConfig {
            ...
            similarity_threshold: -0.5, // Lower threshold for hash-based embeddings
    ...
    pub fn initialize_pagerank(&mut self, graph: &KnowledgeGraph) -> Result<()> {
        ...
        println!("🚀 Initializing high-performance PageRank retrieval system...");
        ...
        println!("✅ PageRank retrieval system initialized with 27x performance optimizations");
        Ok(())
    }
    ...
    pub fn pagerank_query(
        ...
        if let Some(ref pagerank_retriever) = self.pagerank_retriever {
    ...
    pub fn hybrid_query_with_trees(
        ...
        let results = self.execute_adaptive_retrieval(
            query,
            &query_embedding,
            graph,
            document_trees,
    ...
    fn add_embeddings_parallel(
        ...
        if processor.should_use_parallel(total_items) {
            println!("Processing {total_items} embeddings with enhanced sequential approach");
        }
    ...
    fn add_embeddings_sequential(&mut self, graph: &mut KnowledgeGraph) -> Result<()> {
        ...
        println!("Generated embeddings for {chunk_count} chunks and {entity_count} entities");
    ...
    pub fn batch_query(
        ...
        if let Some(ref processor) = self.parallel_processor.clone() {
    ...
    fn calculate_strategy_weights(&self, analysis: &QueryAnalysis) -> (f32, f32, f32) {
        match (&analysis.query_type, &analysis.intent) {
            (QueryType::EntityFocused, _) => (0.3, 0.6, 0.1),
            (QueryType::Relationship, _) => (0.2, 0.7, 0.1),
    ...
    fn hierarchical_search(
        ...
        for tree in document_trees.values() {
            ...
            for tree_result in tree_results {
                ...
                results.push(SearchResult {
                    id: tree_result.node_id.to_string(),
                    content: tree_result.summary,
                    score: tree_result.score + level_bonus,
                    result_type: ResultType::HierarchicalSummary,
                    entities: tree_result.keywords.clone(),
                    source_chunks: tree_result
                        .chunk_ids
                        .iter()
                        .map(|id| id.to_string())
                        .collect(),
    ...
    pub fn save_state_to_json(&self, file_path: &str) -> Result<()> {
        ...
        println!("Retrieval system state saved to {file_path}");
...
impl RetrievalStatistics {
    ...
    pub fn print(&self) {
        println!("Retrieval System Statistics:");
        println!("  Indexed vectors: {}", self.indexed_vectors);
        println!("  Vector dimension: {}", self.vector_dimension);
        ...
        println!("    Entity weight: {:.2}", self.config.entity_weight);
        println!("    Chunk weight: {:.2}", self.config.chunk_weight);
        println!("    Graph weight: {:.2}", self.config.graph_weight);
    }
}
