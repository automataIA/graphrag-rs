...
impl CacheStatistics {
    ...
    pub fn print(&self) {
        ...
        println!("  Requests: {} (hits: {}, misses: {})", metrics.total_requests, metrics.cache_hits, metrics.cache_misses);
        println!("  Hit rate: {:.2}% | Miss rate: {:.2}%", metrics.hit_rate * 100.0, metrics.miss_rate * 100.0);
        println!("  Time saved: {:.2}s total, {:.2}ms avg per hit",
                 metrics.total_time_saved.as_secs_f64(),
                 metrics.avg_time_saved_per_hit.as_secs_f64() * 1000.0);
        println!("  Cache operations: {} insertions, {} evictions, {} updates",
                 metrics.insertions, metrics.evictions, metrics.updates);
        println!("  Current size: {} entries, {} memory", metrics.current_size, metrics.memory_usage_human);
...
impl CacheHealth {
    ...
    pub fn print(&self) {
        ...
        println!("  Efficiency Score: {:.2}", self.metrics.efficiency_score());

        if !self.alerts.is_empty() {
            println!("  Alerts:");
            for alert in &self.alerts {
                println!("    [{:?}] {}: {:.3} (threshold: {:.3})",
                         alert.level, alert.message, alert.current_value, alert.threshold);
            }
        }
