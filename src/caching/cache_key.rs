...
impl CacheKeyGenerator {
    ...
    fn serialize_params(&self, params: &GenerationParams) -> CacheResult<String> {
        ...
        if let Some(ref stop_sequences) = params.stop_sequences {
    ...
    fn create_display_key(
        ...
        if let Some(params) = params {
            if let Some(temp) = params.temperature {
                display_parts.push(format!("T:{temp:.1}"));
            }
        }
...
impl KeyStatistics {
    pub fn print(&self) {
        ...
        println!("  Avg prompt length: {:.1} chars", self.avg_prompt_length);
    }
}
