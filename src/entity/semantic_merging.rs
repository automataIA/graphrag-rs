    ...
    pub async fn decide_merge(&self, entity_group: &[Entity]) -> Result<EntityMergeDecision> {
        ...
        if let Some(ref llm_client) = self.llm_client {
            ...
            match self.try_llm_merge_decision(llm_client, &prompt).await {
                ...
                Err(_) => {
                    println!("⚠️ LLM merge decision failed, falling back to heuristics");
...
impl MergingStatistics {
    pub fn print(&self) {
        println!("🔄 Entity Merging Statistics");
        println!("  Similarity threshold: {:.2}", self.similarity_threshold);
        println!("  Max description tokens: {}", self.max_description_tokens);
        println!("  Uses LLM: {}", self.uses_llm);
