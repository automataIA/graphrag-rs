...
use crate::{
    config::toml_config::EntityExtractionConfig,
...
impl EntityExtractor {
    ...
    pub fn with_config(config: EntityExtractionConfig) -> Result<Self> {
        ...
        if let Some(ref filters) = config.filters {
            if let Some(ref patterns) = filters.allowed_patterns {
                for pattern in patterns {
                    match Regex::new(pattern) {
                        Ok(regex) => allowed_patterns.push(regex),
                        Err(e) => {
                            eprintln!("Warning: Invalid allowed pattern '{pattern}': {e}");
                        }
                    }
                }
            }

            if let Some(ref patterns) = filters.excluded_patterns {
                for pattern in patterns {
                    match Regex::new(pattern) {
                        Ok(regex) => excluded_patterns.push(regex),
                        Err(e) => {
                            eprintln!("Warning: Invalid excluded pattern '{pattern}': {e}");
                        }
                    }
                }
            }
    ...
    pub fn extract_from_chunk(&self, chunk: &TextChunk) -> Result<Vec<Entity>> {
        ...
        let entity_types = if let Some(ref config) = self.config {
